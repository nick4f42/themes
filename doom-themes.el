...
(defun doom-themes--apply-faces (new-faces &optional default-faces)
  (declare (pure t) (side-effect-free t))
  (let ((default-faces
         (or default-faces (append doom-themes-base-faces doom-themes-face-overrides)))
    ...
    (cl-loop for face being the hash-keys of directives
             ...
             (puthash face
                      (let ((old-spec (seq-copy (gethash (or target face) faces)))
                            (plist spec))
                        ;; remove duplicates
...
Variables in EXTRA-VARS override the default ones."
  (declare (pure t) (side-effect-free t))
  (cl-loop for (var val) in (append doom-themes-base-vars doom-themes-var-overrides vars)
